...
threshold = 0.1  # Enter the threshold to activate the robot
push_counter = 0
...
class LiveAdvance():
    ...
    def __init__(self, app_client_id, app_client_secret, **kwargs):
        # Initialize Cortex for communication with Emotiv headset
    ...
    def on_new_com_data(self, *args, **kwargs):
        ...
        if data['action'] == "push" and data['power'] >= threshold:
            push_counter += 1
            print(f"Push command detected, count: {push_counter}")
            # When push_counter reaches 3, send 'w' to Arduino
            if push_counter == 3:
                print("Moving forward")
                arduino.write(b'w')  # Send forward command to Arduino
                time.sleep(1)  # Wait a bit before accepting more commands
                push_counter = 0  # Reset the counter after sending 'w'
        elif data['action'] == "pull" and data['power'] >= threshold:
            print("Moving backward")
            arduino.write(b's')  # Send back command to Arduino
        elif data['action'] == "left" and data['power'] >= threshold:
            print("Moving left")
            arduino.write(b'a')  # Send left command to Arduino
        elif data['action'] == "right" and data['power'] >= threshold:
            print("Moving right")
            arduino.write(b'd')  # Send right command to Arduino
...
#          handle create and load a profile automatically . So you should not modify them
